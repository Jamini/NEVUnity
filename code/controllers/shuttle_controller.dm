...
#define SHUTTLEARRIVETIME 180		// 3 minutes = 180 seconds
...
datum/shuttle_controller/proc/recall()
	if(direction == 1)
		...
		if(alert == 0)
			if(timeleft >= 600)
				return
			captain_announce("The evacuation has been canceled.")
	...
	switch(location)
		if(0)
			...
			if(timeleft>timelimit)
				...
				captain_announce("The Escape pods are primed. You have [round(timeleft()/60,1)] minutes to board the Escape pods.")
		...
		if(1)
			...
			/* --- Shuttle leaves the station, enters transit --- */
				...
				*/
				...
				captain_announce("The Escape Pods have left the station. Estimate [round(timeleft()/60,1)] minutes until the shuttle docks at Central Command.")
