		...
		if("MessageCentcomm")
			if(src.authenticated==2)
				if(centcomm_message_cooldown)
					...
					return
				var/input = stripped_input(usr, "Please choose a message to transmit to the NMV Trioa via quantum entanglement. There is a 30 second delay before you may send another message, be clear, full and concise.", "To abort, send an empty message.", "")
				if(!input || !(usr in view(1,src)))
					return
		...
		if("MessageSyndicate")
			if((src.authenticated==2) && (src.emagged))
				if(centcomm_message_cooldown)
					usr << "\red Arrays recycling.  Please stand by."
					return
				var/input = stripped_input(usr, "Please choose a message to transmit to \[ABNORMAL ROUTING CORDINATES\] via quantum entanglement. There is a 30 second delay before you may send another message, be clear, full and concise.", "To abort, send an empty message.", "")
				if(!input || !(usr in view(1,src)))
					return
		...
		if(STATE_DEFAULT)
			if (src.authenticated)
				...
				if (src.authenticated==2)
					...
					if(src.emagged == 0)
						dat += "<BR>\[ <A HREF='?src=\ref[src];operation=MessageCentcomm'>Send an emergency message to the Trioa</A> \]"
					else
				...
				if(emergency_shuttle.location==0)
					if (emergency_shuttle.online)
						...
						dat += "<BR>\[ <A HREF='?src=\ref[src];operation=callshuttle'>Call an Evacuation</A> \]"
	...
	switch(src.aistate)
		if(STATE_DEFAULT)
			if(emergency_shuttle.location==0 && !emergency_shuttle.online)
				dat += "<BR>\[ <A HREF='?src=\ref[src];operation=ai-callshuttle'>Call Evacuation</A> \]"
...
/proc/call_shuttle_proc(var/mob/user)
	...
	if(sent_strike_team == 1)
		user << "Remote shutdown of all escape pods activated. Consider all contracts terminated."
		return

	if(world.time < 6000) // Ten minute grace period to let the game get going without lolmetagaming. -- TLE
		user << "An evacuation cannot be called. Please wait another [round((6000-world.time)/600)] minutes before trying again."
		return

	if(emergency_shuttle.direction == -1)
		user << "All pods are away."
		return

	if(emergency_shuttle.online)
		user << "An Evacuation has already been called."
		return
	...
	captain_announce("An Evacuation has been called. Escape pods will depart in [round(emergency_shuttle.timeleft()/60)] minutes.")
...
/proc/init_shift_change(var/mob/user, var/force = 0)
	...
	if(emergency_shuttle.direction == -1)
		user << "The Escape Pods have already left the ship."
		return
	...
	if(!force)
		if(emergency_shuttle.deny_shuttle)
			user << "Mechanical Error. An Evacuation may not be called Please try again later."
			return
...
/obj/machinery/computer/communications/Del()
	...
	captain_announce("An Evacuation has been sounded. All pods depart in [round(emergency_shuttle.timeleft()/60)] minutes.")
	world << sound('sound/AI/shuttlecalled.ogg')
