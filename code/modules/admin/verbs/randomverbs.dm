...
proc/cmd_admin_mute(mob/M as mob, mute_type, automute = 0)
	if(automute)
		...
		if(!usr || !usr.client)
			return
		if(!usr.client.holder)
			usr << "<font objcolor='red'>Error: cmd_admin_mute: You don't have permission to do this.</font>"
			return
		if(!M.client)
			usr << "<font objcolor='red'>Error: cmd_admin_mute: This mob doesn't have a client tied to it.</font>"
		if(M.client.holder)
			usr << "<font objcolor='red'>Error: cmd_admin_mute: You cannot mute an admin.</font>"
	if(!M.client)		return
...
/proc/create_xeno(ckey)
	if(!ckey)
		...
		if(candidates.len)
			...
			usr << "<font objcolor='red'>Error: create_xeno(): no suitable candidates.</font>"
...
/client/proc/respawn_character()
	...
	if(!G_found)//If a ghost was not found.
		usr << "<font objcolor='red'>There is no active key like that in the game or the person is not currently a ghost.</font>"
		return
