...
/client/proc/strike_team()
	if(!ticker)
		usr << "<font objcolor='red'>The game hasn't started yet!</font>"
		return
	if(world.time < 6000)
		usr << "<font objcolor='red'>There are [(6000-world.time)/10] seconds remaining before it may be called.</font>"
		return
	if(sent_strike_team == 1)
		usr << "<font objcolor='red'>CentCom is already sending a team.</font>"
		return
...
/mob/living/carbon/human/proc/equip_death_commando(leader_selected = 0)
	...
	if (leader_selected == 0)
		equip_to_slot_or_del(new /obj/item/clothing/under/objcolor/green(src), slot_w_uniform)
	else
